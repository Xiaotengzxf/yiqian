...
{
    view_Dele_Button_Bg = [[UIView alloc]init];
    
    view_Dele_Button_Bg.frame = CGRectMake(0, 0, 74, 64);
    
    view_Dele_Button_Bg.backgroundColor = [UIColor clearColor];
    
    UIImageView *image_Back = [[UIImageView alloc]init];
    
    image_Back.frame = CGRectMake(17, 29, 10, 18);
    
    image_Back.image = [UIImage imageNamed:@"pic_stone_details_back_white.png"];
    
    image_Back.userInteractionEnabled = YES;

    UIButton *button_Back_Rem = [UIButton buttonWithType:UIButtonTypeCustom];
    
    button_Back_Rem.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height);
    
    [button_Back_Rem addTarget:self action:@selector(button_back_Rem_click) forControlEvents:UIControlEventTouchUpInside];
    
    button_Back_Rem.backgroundColor = [UIColor clearColor];

    [self addSubview:image_Back];

    [self addSubview:button_Back_Rem];
    
    [self addSubview:view_Dele_Button_Bg];
...
//    _upView.frame = CGRectMake((ScreenWidth - scanViewW) * 0.5, 0, scanViewW, 150);
    ...
    //扫码框
    _centerView.frame = CGRectMake(CGRectGetMaxX(_leftView.frame), CGRectGetMaxY(_upView.frame), scanViewW, [UIScreen mainScreen].bounds.size.height - 64 -49);
    //扫描线
...
-(void)button_back_Rem_click {
    
    [view_Dele_Button_Bg removeFromSuperview];
